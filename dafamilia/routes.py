...
from flask_login import login_user, logout_user, current_user, login_required  # pylint: disable=unused-import
from PIL import Image
from dafamilia import app, database, bcrypt
from dafamilia.forms import FormLogin, FormCriarConta, FormEditarPerfil, FormCriarPost
from dafamilia.models import Usuario, Post
...
def login():
    ...
    if 'botao_submit_login' in request.form:
        if form_login.validate_on_submit():
            ...
            if usuario and bcrypt.check_password_hash(usuario.senha, form_login.senha.data):
                ...
                flash(
                    f'Login feito com sucesso no email: {form_login.email.data}', 'alert-success')
    ...
    if 'botao_submit_criar_conta' in request.form:
        if form_criar_conta.validate_on_submit():
            ...
            usuario = Usuario(username=form_criar_conta.username.data,
                              email=form_criar_conta.email.data, senha=senha_crypt)
            database.session.add(usuario)
            database.session.commit()
            flash(
                f'Conta criada com sucesso no email: {form_criar_conta.email.data}', 'alert-success')  # pylint: disable=line-too-long
            return redirect(url_for('home'))
...
def editar_perfil():
    ...
    if form.validate_on_submit():
        ...
        database.session.commit()
        flash(
            'Perfil atualizado com sucesso', 'alert-success')
